...
class GetSeries {
    ...
    fun execute(url: String, userAgent: String): Series {
        ...
        if (doc.isSuccess) {
            val seriaParce = doc.getOrThrow().select("a[class=\"short-btn black video the_hildi\"]")
                .textNodes().map { it.toString() }
            val seriaParceGreen = doc.getOrThrow().select("a[class=\"short-btn green video the_hildi\"]")
                .textNodes().map { it.toString() }
